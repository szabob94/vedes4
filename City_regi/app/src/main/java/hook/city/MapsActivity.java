...
public class MapsActivity extends FragmentActivity {
    ...
    String [] busz_szama=new String[30000];
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int f=0;f<i;f++){
            if(bus_way_coords_lon[f].equals("busz") || bus_way_coords_lon[f].equals("(Airport)") || bus_way_coords_lon[f].equals("György")){
                latok.add(new ArrayList<ArrayList<String>>(waylatok));
                lonok.add(new ArrayList<ArrayList<String>>(waylonok));
                busz.add(new String (busz_szama[f]));
                waylatok.clear();
                waylonok.clear();
            }
            else{
                if(busz_szama[f].equals("way")){
                waylatok.add(new ArrayList(lat));
                waylonok.add(new ArrayList(lon));
                //lon.add(bus_way_coords_lon[f]);
                lat.clear();
                lon.clear();}
                else{
                    lat.add(busz_szama[f]);
                    lon.add(bus_way_coords_lon[f]);
                }
            }
        }
    ...
    public void chooseBus(View view) {
        ...
        ((android.widget.Button)findViewById(R.id.chooseBus)).setText(busz.get(hanyas));
        hanyas++;
        if(hanyas==busz.size()){
            hanyas=0;
        }
